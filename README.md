...
승차 시점에 알 수 있는 정보만으로 **해당 트립이 정체 구간에 걸릴지** 예측합니다.
...
모아 비교**했습니다. 이 저장소는 그중 '정체 예측' 가설의 파이프라인입니다.
...
정체의 정의는 하나로 정하지 않고 **두 가지를 함께 학습해 비교**합니다.
...
두 정의를 비교하는 이유는 **점수가 높은 가설이 곧 좋은 가설은 아니라는 점**을 확인하기 위해서입니다.
...
실행 시간은 약 15초이며, 완료 후 `outputs/` 하위에 모든 산출물이 생성됩니다.

정적 검사는 `ruff check main.py src/` 로 수행합니다(설정은 `pyproject.toml`).
...
**5개 컬럼의 결측 위치가 완전히 일치**했습니다. 크기는 **955,371행(23.4%)** 입니다.

블록 안팎의 벤더 구성도 겹치지 않습니다 — **VendorID 6은 결측 블록에만, 7은 나머지에만** 존재합니다.
즉 값이 개별적으로 빠진 것이 아니라 **서로 다른 데이터 파이프라인이 섞여 있었던 것**입니다.
평균·최빈값 대체는 존재하지 않는 값을 지어내는 셈이므로 이 블록은 제거했습니다.

### NaN 이 아닌 코드값 결측

`RatecodeID = 99`(unknown, 3.44%)와 존 ID `264/265`(Unknown/N-A, 0.62%)는
약속된 코드값이라 `isna()`로 잡히지 않습니다. 존 미상은 핵심 피처가 비는 것이라 제거했고,
`RatecodeID = 99`는 '미상'이라는 사실 자체가 정보이므로 별도 범주로 남겼습니다.
...
절대정체가 점수는 높지만, 그 차이의 상당 부분은 **'도심 단거리인지'를 맞히는 데서 옵니다**
(절대정체로 분류된 트립의 39.2%가 1마일 미만, 상대정체는 29.5%).
상대정체는 경로 성격을 통제하므로 점수는 낮아도 '평소보다 막혔다'는 의미에 더 충실합니다.
...
- 원본 데이터(약 70MB)와 학습된 모델 파일은 용량 문제로 git에 포함하지 않습니다(`.gitignore` 참고).
