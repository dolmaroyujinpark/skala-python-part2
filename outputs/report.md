...
| 방식 | 소요 시간(초) |
|---|---|
| Pandas `read_parquet` | 0.248 |
| Polars `read_parquet` (Eager) | 0.141 |
| Polars `scan_parquet` (Lazy, 필요 컬럼만) | 0.106 |

Polars가 Pandas 대비 **1.76배** 빠르게 읽었다.
...
- Polars `scan_parquet` + `group_by` : 0.106초
- Pandas `groupby` : 0.216초
...
|---|---|---|---|---|---|---|
| jam_abs | 절대정체 (전체 트립 평균속도 하위 33%) | 0.7964 | 0.7651 | 0.3966 | 0.3685 | 3.6000 |
| jam_rel | 상대정체 (동일 경로 평소 속도 대비 하위 33%) | 0.7339 | 0.6575 | 0.4002 | 0.2573 | 3.8000 |
